...
- 平文 HTTP は Authorization / Cookie / PII を含み、TLS でも SNI / ClientHello / 接続パターンが残る。`/tmp` 直書きは world-readable で事故源
- 書き出し先は専用 dir を 700 で作成: `install -d -m 700 /var/tmp/pcap-$USER`
- 解析後 `shred -u`（macOS は `rm -P`）で破棄。S3 等共有 storage に上げない
- header のみで足りるなら `-s 256` 等で payload を切り詰める ([tcpdump(1)](https://www.tcpdump.org/manpages/tcpdump.1.html))。Slack / Issue に貼るときは `editcap -F pcap -s 64 in.pcap out.pcap` で更に切り詰める

### HTTP/3 (QUIC) を含める場合

- HTTP/3 は UDP 443 上で TLS handshake が走る。`'tcp port 443'` filter では pass-through
- capture filter を `'(tcp or udp) port 443'` にする
- display filter は `quic.tls.handshake.*` (例: `quic.tls.handshake.extensions_server_name`) を使う
