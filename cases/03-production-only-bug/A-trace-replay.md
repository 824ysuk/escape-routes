...
# logs に body を載せている場合は /api/v2/logs/events/search に差し替える
...
# 2. request body を抽出（field path は APM の log/span structure 依存）
# 例: .data[].attributes.custom.http.request.body などにマップされていることもある
...
# 3d. gitleaks で派生 file を 2 重チェック (allow-list / deny-list の漏れを検出)
gitleaks detect --no-git --source payload-allowlist.json --report-format=json --report-path=gitleaks-report.json || \
  { echo 'secret 検出。allow-list の field 定義を見直す'; exit 1; }
