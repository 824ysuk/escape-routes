...
```bash
# pgrep -f gunicorn は master + N workers の複数 PID を返す。
# master 1 つを指定して --subprocesses で worker を集約するのが安定。
APP_PID=$(pgrep -of gunicorn)   # 最古プロセス = master
...
#   --subprocesses : gunicorn / uwsgi の worker をまとめて取る (master から fork した child を follow)
