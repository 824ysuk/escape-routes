...
> **本 doc の取り込み経緯**: 本 doc は並行 session の PR #19 (ホワイトハッカーパネルレビュー版) で生成された。最終的には PR #18 (critical/high accuracy fix 版) を base に採用し、本 doc は repo の findings anchor として cherry-pick された。そのため doc 内の「本レビューで対応済 / 未対応」の境界は PR #19 を base にした場合の整理であり、PR #18 base 基準では対応 case が異なる (例: 03/A の Datadog Spans API path 訂正 / 03/B の `pgrep` 複数 PID 修正 / 04/A の csv-stringify columns / pandas chunksize header は PR #18 で対応済)。境界の最新整理は今後の cherry-pick PR / Issue 化で実施する。
