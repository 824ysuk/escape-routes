...
各代替手段ファイル（`X-<short>.md`）の冒頭、タイトル直下に以下の metadata block を必須とする。markdown blockquote（`>` 始まり）で記述することで GitHub レンダリング上も視認しやすくなる。

```markdown
# 事例 N-X: <手段名>

> **対象範囲**: <自分のアカウント / 自社サービス / 公開情報 等、利用可能なスコープを 1 文>
> **第三者データ**: <扱う / 扱わない / 利用者の同意下のみ>
> **PII**: <扱う / 扱わない / 一時的に通過する>
> **適法境界**: <根拠法と該当条文 / 該当 ToS / 不明>
> **Disclosure 経路**: <脆弱性発見時の連絡先、または「N/A (脆弱性発見性なし)」>
...
| フィールド | 記述内容 |
|---|---|
| 対象範囲 | 誰のデータ・どのサービスを対象にできるか |
| 第三者データ | 本人以外のデータを扱うかどうか |
| PII | 個人識別情報が経路を通過するかどうか |
| 適法境界 | 根拠となる法令・ToS の条文または「不明」 |
| Disclosure 経路 | 脆弱性を発見した場合の連絡先 |

既存の代替手段ファイルへの遡及適用は段階移行で対応する（本 CONTRIBUTING.md への追記が先、既存ファイルへの適用は別途）。
